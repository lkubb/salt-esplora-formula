...
We will see a quick setup to configure the ``esplora`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``esplora`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``esplora`` formulas under ``/srv/salt/esplora/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/esplora/parameters/dns:domain/
    ln -s dns:domain /srv/salt/esplora/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/esplora/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/esplora/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``esplora`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``esplora`` formulas under ``/srv/salt/esplora/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/esplora/parameters/roles
...
We create a configuration for the role ``esplora/server`` in ``/srv/salt/esplora/parameters/roles/esplora/server.yaml``:
...
We create another configuration for the role ``esplora/client`` in ``/srv/salt/esplora/parameters/roles/esplora/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles esplora/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles esplora/client
...
.. code-block:: console
    ...
    mv /srv/salt/esplora/parameters/dns:domain/  '/srv/salt/esplora/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-esplora-as-json:
      test.nop:
        - name: {{ esplora | json }}
...
Here is an example based on `template-formula/esplora/config/file.sls`_:
...
This ``sls`` file expose a ``esplora`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ esplora['winner'] }}
...
.. _template-formula/esplora/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/esplora/config/file.sls
